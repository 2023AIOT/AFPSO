...
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), 'fuzzy_pso'))

from fuzzy_pso import EnergyPredictor, AdaptiveFuzzySystem, RuleWeightPSO, DistributedPSOManager
# =================================================

# 混沌映射 - Logistic 映射生成器
def logistic_chaos(seed: float = 0.42, r: float = 3.99):
    """生成 (0,1) 上的 logistic 混沌序列生成器。seed 取 (0,1) 内非特殊值。"""
    x = float(seed)
    while True:
        x = r * x * (1.0 - x)
        # 避免边界值
...
# 能量跟踪配置（用于外部实验记录特定节点能耗曲线）
_ENERGY_TRACE_CONFIG = None


def enable_energy_trace(node_indices: List[int]):
    """
    启用节点能量跟踪。node_indices为需要跟踪的节点索引列表。
    """
    global _ENERGY_TRACE_CONFIG
    filtered = sorted({idx for idx in node_indices if isinstance(idx, int) and idx >= 0})
    _ENERGY_TRACE_CONFIG = {'node_indices': filtered} if filtered else None


def disable_energy_trace():
    """关闭能量跟踪并清理缓存。"""
    global _ENERGY_TRACE_CONFIG
    _ENERGY_TRACE_CONFIG = None


def consume_energy_trace():
    """
    读取 run_improve_leach 最近一次运行产生的能量跟踪结果，并清理缓存。
    返回结构: {'rounds': [...], 'node_records': {idx: [...]} , 'initial_power': {idx: float}}
    若未启用跟踪则返回None。
    """
    global _ENERGY_TRACE_CONFIG
    if _ENERGY_TRACE_CONFIG is None:
        return None
    result = _ENERGY_TRACE_CONFIG.get('result')
    _ENERGY_TRACE_CONFIG = None
    return result
...
BASE_DEPLOYMENT_PARAMS = {
    ...
    'sink_y': sink.y,
    'Rmax': Rmax,
    'min_sensor_nodes': min_sensor_nodes,
...
class PSOOptimizer:
    """
    粒子群优化算法类，用于优化模糊系统的隶属函数参数
    """
    def __init__(self, n_particles=20, n_iterations=10, w=0.7, c1=1.5, c2=1.5, 
                 bounds=None, fitness_func=None, param_subset=None,
                 use_chaos: bool = False, chaos_seed: float = None, chaos_r: float = 3.99,
                 w_min: float = 0.4):
        """
        初始化PSO优化器
        参数:
            ...
            c1: 个体学习因子
            c2: 社会学习因子
            bounds: 参数边界，格式为 {param_name: (min, max)} 的字典
            fitness_func: 适应度函数
        """
        ...
        if chaos_seed is None:
            try:
                ...
                chaos_seed = 0.42
        self.chaos_seed = float(chaos_seed)
        self.chaos_r = float(chaos_r)
        # 创建两个独立的混沌序列生成器用于 r1, r2（种子偏移）
        if self.use_chaos:
            self._chaos1 = logistic_chaos(self.chaos_seed, self.chaos_r)
            self._chaos2 = logistic_chaos((self.chaos_seed + 0.123456) % 1.0, self.chaos_r)
            # 单独为 gbest 扰动准备第三条混沌序列，避免与 r1/r2 串联相关
            self._chaos3 = logistic_chaos((self.chaos_seed + 0.654321) % 1.0, self.chaos_r)
        else:
            self._chaos1 = None
            self._chaos2 = None
            self._chaos3 = None
        # 动态惯性权重的下界（线性递减 w -> w_min）
        self.w_max = float(self.w)
        self.w_min = float(w_min)
        
        # 定义参数边界（每个trimf有3个参数）
        if bounds is None:
            self.bounds = {
                'energy_low': [(0, 0.2), (0, 0.3), (0.3, 0.5)],
                'energy_medium': [(0.1, 0.3), (0.4, 0.6), (0.7, 0.9)],
                'energy_high': [(0.5, 0.7), (0.9, 1.0), (1.0, 1.0)],
                'neighbor_sparse': [(0, 0.2), (0, 0.3), (0.3, 0.5)],
                'neighbor_moderate': [(0.1, 0.3), (0.4, 0.6), (0.7, 0.9)],
                'neighbor_dense': [(0.5, 0.7), (0.9, 1.0), (1.0, 1.0)],
                'distance_near': [(0, 0.2), (0, 0.3), (0.3, 0.5)],
                'distance_medium': [(0.1, 0.3), (0.4, 0.6), (0.7, 0.9)],
                'distance_far': [(0.5, 0.7), (0.9, 1.0), (1.0, 1.0)],
                'chance_very_low': [(0, 0.1), (0, 0.2), (0.2, 0.3)],
                'chance_low': [(0, 0.2), (0.2, 0.3), (0.4, 0.6)],
                'chance_medium': [(0.2, 0.4), (0.4, 0.6), (0.6, 0.8)],
                'chance_high': [(0.5, 0.7), (0.7, 0.8), (0.9, 1.0)],
                'chance_very_high': [(0.7, 0.8), (0.9, 1.0), (1.0, 1.0)]
            }
        else:
            self.bounds = bounds
        
        # 参数名称列表（支持对子集优化）
        all_param_names = list(self.bounds.keys())
        if param_subset is not None:
            # 仅保留在边界定义内且用户指定的参数
            self.param_names = [name for name in all_param_names if name in param_subset]
            # 若用户提供的子集无效，则回退到全部参数
            ...
            self.param_names = all_param_names
        # 计算总维度（每个参数有3个值）
        self.dim = len(self.param_names) * 3
        # 保存子集供外部检查
        self.param_subset = list(self.param_names)
    ...
    def _initialize_particles(self, initial_params=None, perturbation_scale=0.1):
        """
        初始化粒子位置和速度
        参数:
            initial_params: 历史最优参数字典，如果提供则基于此初始化
            perturbation_scale: 扰动比例，用于在历史最优参数附近生成粒子
        """
        self.particles = []
        self.velocities = []
        ...
        if initial_params is not None:
            ...
            for param_name in self.param_names:
                if param_name in initial_params:
                    param_values = initial_params[param_name]
                    # 确保 param_values 是长度为3的列表
                    if isinstance(param_values, (list, tuple, np.ndarray)):
                        # 如果是列表/数组，直接使用
                        for i in range(3):
                            initial_particle.append(float(param_values[i]))
                    else:
                        # 如果是标量，使用边界中点
                        min_val, max_val = self.bounds[param_name][0]
                        mid_val = (min_val + max_val) / 2
                        for i in range(3):
                            initial_particle.append(mid_val)
                else:
                    # 如果参数缺失，使用边界中点
                    for i in range(3):
                        min_val, max_val = self.bounds[param_name][i]
                        initial_particle.append((min_val + max_val) / 2)
            initial_particle = np.array(initial_particle)
        
        for i in range(self.n_particles):
            if initial_particle is not None and i == 0:
                # 第一个粒子使用历史最优参数
                particle = initial_particle.copy()
            elif initial_particle is not None:
                # 其他粒子在历史最优参数附近扰动
                particle = []
                for j, param_name in enumerate(self.param_names):
                    for k in range(3):
                        min_val, max_val = self.bounds[param_name][k]
                        base_val = initial_particle[j * 3 + k]
                        # 在历史最优值附近添加小扰动
                        perturbation = random.uniform(-perturbation_scale * (max_val - min_val),
                                                      perturbation_scale * (max_val - min_val))
                        new_val = base_val + perturbation
                        # 确保在边界内
                        new_val = max(min_val, min(max_val, new_val))
                        particle.append(new_val)
                ...
                for param_name in self.param_names:
                    for j in range(3):
                        min_val, max_val = self.bounds[param_name][j]
                        if self.use_chaos:
                            # 使用混沌序列生成初始位置（缩放到边界区间）
                            rand_val = next(self._chaos1)
                            particle.append(min_val + rand_val * (max_val - min_val))
                            vel_range = (max_val - min_val) * 0.1
                            vel_rand = next(self._chaos2)
                            velocity.append(-vel_range + vel_rand * (2 * vel_range))
                        else:
                            particle.append(random.uniform(min_val, max_val))
                            velocity.append(random.uniform(-(max_val - min_val) * 0.1, 
                                                          (max_val - min_val) * 0.1))
                particle = np.array(particle)
                velocity = np.array(velocity)
            
            # 初始化速度（增量学习时速度应该较小）
            if initial_particle is not None and i > 0:  # 第一个粒子已经在上面处理了
                velocity = []
                for j, param_name in enumerate(self.param_names):
                    for k in range(3):
                        min_val, max_val = self.bounds[param_name][k]
                        # 增量学习时速度范围更小
                        velocity.append(random.uniform(-perturbation_scale * (max_val - min_val) * 0.5,
                                                      perturbation_scale * (max_val - min_val) * 0.5))
                velocity = np.array(velocity)
            elif initial_particle is not None and i == 0:
                # 第一个粒子使用历史最优，速度初始化为0或很小的值
                velocity = np.zeros_like(initial_particle)
            
            self.particles.append(particle)
            self.velocities.append(velocity)
        
        # 初始化个体最优
        self.pbest_positions = [p.copy() for p in self.particles]
        self.pbest_fitness = [float('-inf')] * self.n_particles
        
    def _particle_to_params(self, particle):
        """将粒子位置转换为参数字典"""
        params = {}
        idx = 0
        for param_name in self.param_names:
            params[param_name] = [
                max(self.bounds[param_name][0][0], 
                    min(self.bounds[param_name][0][1], particle[idx])),
                max(self.bounds[param_name][1][0], 
                    min(self.bounds[param_name][1][1], particle[idx + 1])),
                max(self.bounds[param_name][2][0], 
                    min(self.bounds[param_name][2][1], particle[idx + 2]))
            ]
            idx += 3
        return params
    ...
    def _evaluate_fitness(self, particle):
        ...
        params = self._particle_to_params(particle)
        return self.fitness_func(params)
    
    def optimize(self, initial_params=None, incremental_mode=False, perturbation_scale=0.1):
        """
        执行PSO优化
        参数:
            initial_params: 历史最优参数字典，用于增量式学习
            incremental_mode: 是否为增量式学习模式
            perturbation_scale: 扰动比例，仅在增量模式下有效
        返回:
            优化后的参数字典
        """
        # 初始化粒子群（如果提供历史参数，则基于其初始化）
        self._initialize_particles(initial_params=initial_params, 
                                   perturbation_scale=perturbation_scale)
        
        # 如果是增量模式，使用历史最优作为初始全局最优（仅微调）
        if incremental_mode and initial_params is not None:
            initial_particle = []
            for param_name in self.param_names:
                if param_name in initial_params:
                    param_values = initial_params[param_name]
                    for i in range(3):
                        initial_particle.append(param_values[i])
                else:
                    for i in range(3):
                        min_val, max_val = self.bounds[param_name][i]
                        initial_particle.append((min_val + max_val) / 2)
            self.gbest_position = np.array(initial_particle)
            self.gbest_fitness = self._evaluate_fitness(self.gbest_position)
        
        # 评估初始适应度
        for i in range(self.n_particles):
            fitness = self._evaluate_fitness(self.particles[i])
            self.pbest_fitness[i] = fitness
            if fitness > self.gbest_fitness:
                self.gbest_fitness = fitness
                self.gbest_position = self.particles[i].copy()
        
        prev_gbest = float(self.gbest_fitness)
        stagnation_counter = 0

        # 迭代优化（自适应权重 & 基于混沌的扰动以避免早熟）
        for iteration in range(self.n_iterations):
            # 线性递减基准惯性权重
            if self.n_iterations > 1:
                w_base = self.w_max - (self.w_max - self.w_min) * (iteration / float(self.n_iterations - 1))
            else:
                w_base = self.w_max

            # 计算群体多样性（按各维度 std 的均值归一化）
            try:
                div = float(np.mean(np.std(self.particles, axis=0)))
            except Exception:
                div = 0.0
            # 估算参数尺度（用于归一化多样性）
            param_spans = []
            for param_name in self.param_names:
                # 每个 trimf 有三个控制点，取平均跨度作为该参数的尺度估计
                spans = []
                for j in range(3):
                    min_v, max_v = self.bounds[param_name][j]
                    spans.append(abs(max_v - min_v))
                param_spans.append(np.mean(spans) if spans else 1.0)
            mean_span = float(np.mean(param_spans)) if param_spans else 1.0
            diversity_norm = min(1.0, div / (mean_span + 1e-12))

            # 计算适应度变化用于检测停滞
            fitness_change = abs(self.gbest_fitness - prev_gbest) / (abs(prev_gbest) + 1e-12)

            # 自适应调整惯性权重与学习因子（早期更偏探索，后期更偏利用）
            exploration_factor = diversity_norm  # 当群体多样性高时偏向探索
            w_adjust_base = w_base * (0.5 + 0.5 * exploration_factor)
            c1_dynamic = self.c1 * (1.0 - 0.5 * exploration_factor)  # 个体认知分量随探索减小
            c2_dynamic = self.c2 * (0.5 + 0.5 * exploration_factor)  # 社会分量随探索增加

            # 若增量模式，缩小学习因子幅度与惯性权重以保证微调性质
            if incremental_mode:
                c1_dynamic *= 0.5
                c2_dynamic *= 0.5
                w_adjust_base *= 0.5
            ...
            for i in range(self.n_particles):
                # 生成 r1, r2：优先使用混沌序列以增强早期均匀探索
                if self.use_chaos and self._chaos1 is not None and self._chaos2 is not None:
                    r1 = next(self._chaos1)
                    r2 = next(self._chaos2)
                else:
                    r1 = random.random()
                    r2 = random.random()
                
                # 速度更新（自适应因子）
                self.velocities[i] = (w_adjust_base * self.velocities[i] +
                                     c1_dynamic * r1 * (self.pbest_positions[i] - self.particles[i]) +
                                     c2_dynamic * r2 * (self.gbest_position - self.particles[i]))
                ...
                for param_name in self.param_names:
                    for j in range(3):
                        min_val, max_val = self.bounds[param_name][j]
                        if self.particles[i][idx] < min_val:
                            self.particles[i][idx] = min_val
                            self.velocities[i][idx] = 0
                        elif self.particles[i][idx] > max_val:
                            self.particles[i][idx] = max_val
                            self.velocities[i][idx] = 0
                        idx += 1
            ...
            # 检测停滞：若全局适应度变化很小且多样性也较低，则用混沌扰动激活一部分粒子
            if fitness_change < 1e-6 and diversity_norm < 0.05:
                stagnation_counter += 1
            else:
                stagnation_counter = 0

            if stagnation_counter >= 5 and self.use_chaos:
                # 对 10% 的粒子做小幅混沌扰动以跳出局部极值
                num_perturb = max(1, int(0.1 * self.n_particles))
                for _ in range(num_perturb):
                    pid = random.randrange(self.n_particles)
                    noise = []
                    for _ in range(len(self.particles[pid])):
                        if self._chaos3 is not None:
                            cv = next(self._chaos3)
                        else:
                            cv = random.random()
                        noise.append((cv - 0.5) * 2.0 * (mean_span * 0.02))
                    noise = np.array(noise)
                    self.particles[pid] = self.particles[pid] + noise
                    # re-clip to bounds
                    idx = 0
                    for param_name in self.param_names:
                        for j in range(3):
                            min_val, max_val = self.bounds[param_name][j]
                            if self.particles[pid][idx] < min_val:
                                self.particles[pid][idx] = min_val
                            elif self.particles[pid][idx] > max_val:
                                self.particles[pid][idx] = max_val
                            idx += 1
                stagnation_counter = 0
        ...
        if self.gbest_position is not None:
            return self._particle_to_params(self.gbest_position)
        return None


def create_fuzzy_system(params=None):
    """
    创建模糊逻辑系统
    参数:
        params: 隶属函数参数字典，如果为None则使用默认参数
                格式: {
                    'energy_low': [a, b, c],
                    'energy_medium': [a, b, c],
                    'energy_high': [a, b, c],
                    'neighbor_sparse': [a, b, c],
                    'neighbor_moderate': [a, b, c],
                    'neighbor_dense': [a, b, c],
                    'distance_near': [a, b, c],
                    'distance_medium': [a, b, c],
                    'distance_far': [a, b, c],
                    'chance_very_low': [a, b, c],
                    'chance_low': [a, b, c],
                    'chance_medium': [a, b, c],
                    'chance_high': [a, b, c],
                    'chance_very_high': [a, b, c]
                }
    """
    # 默认参数
    default_params = {
        'energy_low': [0, 0, 0.4],
        'energy_medium': [0.2, 0.5, 0.8],
        'energy_high': [0.6, 1, 1],
        'neighbor_sparse': [0, 0, 0.4],
        'neighbor_moderate': [0.2, 0.5, 0.8],
        'neighbor_dense': [0.6, 1, 1],
        'distance_near': [0, 0, 0.4],
        'distance_medium': [0.2, 0.5, 0.8],
        'distance_far': [0.6, 1, 1],
        'chance_very_low': [0, 0, 0.25],
        'chance_low': [0, 0.25, 0.5],
        'chance_medium': [0.25, 0.5, 0.75],
        'chance_high': [0.5, 0.75, 1],
        'chance_very_high': [0.75, 1, 1]
    }
    ...
    # 为输入变量定义模糊集（使用参数）
    energy['low'] = fuzz.trimf(energy.universe, params['energy_low'])
    energy['medium'] = fuzz.trimf(energy.universe, params['energy_medium'])
    energy['high'] = fuzz.trimf(energy.universe, params['energy_high'])
    
    neighbor_density['sparse'] = fuzz.trimf(neighbor_density.universe, params['neighbor_sparse'])
    neighbor_density['moderate'] = fuzz.trimf(neighbor_density.universe, params['neighbor_moderate'])
    neighbor_density['dense'] = fuzz.trimf(neighbor_density.universe, params['neighbor_dense'])
    
    distance['near'] = fuzz.trimf(distance.universe, params['distance_near'])
    distance['medium'] = fuzz.trimf(distance.universe, params['distance_medium'])
    distance['far'] = fuzz.trimf(distance.universe, params['distance_far'])
    
    # 为输出变量定义模糊集（使用参数）
    chance['very_low'] = fuzz.trimf(chance.universe, params['chance_very_low'])
    chance['low'] = fuzz.trimf(chance.universe, params['chance_low'])
    chance['medium'] = fuzz.trimf(chance.universe, params['chance_medium'])
    chance['high'] = fuzz.trimf(chance.universe, params['chance_high'])
    chance['very_high'] = fuzz.trimf(chance.universe, params['chance_very_high'])
...
# 自动选择敏感参数子集（用于缩小 PSO 搜索空间）
def build_rule_based_candidates() -> Dict[str, float]:
    """
    基于规则参与度的候选参数集构建

    分析9条模糊规则中每个参数的参与度，返回参数名到规则参与度权重的映射。

    返回:
        dict: 参数名到规则参与度分数的映射
    """
    # 定义规则库中每个参数的参与度（基于9条规则的统计）
    rule_participation = {
        'energy_high': 4,      # 规则1,2,6,9
        'energy_medium': 3,    # 规则3,5,7
        'energy_low': 2,       # 规则4,8
        'distance_near': 3,    # 规则1,2,7
        'distance_medium': 2,  # 规则3,6
        'distance_far': 2,     # 规则4,9
        'neighbor_moderate': 3, # 规则1,3,6
        'neighbor_dense': 3,   # 规则2,7,9
        'neighbor_sparse': 2,  # 规则5,8
        # 输出参数通常不优化（除非特别需要）
        'chance_very_low': 0,
        'chance_low': 0,
        'chance_medium': 0,
    ...
    # 归一化到[0,1]区间
    max_participation = max(rule_participation.values())
    if max_participation > 0:
        normalized_participation = {
            param: score / max_participation
            for param, score in rule_participation.items()
        }
    else:
        normalized_participation = rule_participation

    return normalized_participation


def calculate_perturbation_sensitivity(nodes, max_energy, candidate_params,
                                      epsilon=0.01, sample_size=20) -> Dict[str, float]:
    """
    改进的扰动型局部敏感度分析

    主要改进：
    1. 仅对当前输入下被激活的隶属函数进行扰动
    2. 使用相对尺度扰动（隶属函数跨度的1%-5%）
    3. 优先扰动隶属函数的中心参数（第1个控制点）
    4. 避免数值截断，确保扰动直接作用于推理系统
    5. 在多个代表性网络状态上取平均

    参数:
        nodes: 节点列表，用于采样计算敏感度
        max_energy: 最大能量值
        candidate_params: 候选参数字典
        epsilon: 扰动比例，默认1%（相对于隶属函数跨度）
        sample_size: 采样节点数量，默认20
    ...
    for param_name in candidate_params.keys():
        ...
        # Use representative input grid to probe fuzzy response (avoids node-sample plateau)
        test_inputs = [
            (0.05, 0.05, 0.95),
            (0.2, 0.2, 0.7),
            (0.4, 0.5, 0.4),
        ...
        param_values = current_params[param_name]
        param_sensitivity_acc = 0.0
        perturbation_count = 0

        for ti_idx, (norm_energy, norm_neighbors, norm_dist) in enumerate(test_inputs):
            try:
                fuzzy_system = create_fuzzy_system(current_params)
                ...
                base_output = float(fuzzy_system.output['chance'])
            except Exception:
                base_output = float(np.mean([norm_energy, norm_neighbors, norm_dist]))

            if ti_idx < 3:
                print(f"DEBUG_TESTINPUT param={param_name} input#{ti_idx} e={norm_energy:.3f} n={norm_neighbors:.3f} d={norm_dist:.3f} base={base_output:.6f}")

            # perturb central point then one boundary (max 2)
            made = 0
            for i in [1, 0, 2]:
                if made >= 2:
                    break
                ...
                perturbed_params[param_name] = perturbed_values

                try:
                    perturbed_system = create_fuzzy_system(perturbed_params)
                    perturbed_system.input['energy'] = norm_energy
                    perturbed_system.input['neighbor_density'] = norm_neighbors
                    perturbed_system.input['distance'] = norm_dist
                    perturbed_system.compute()
                    perturbed_output = float(perturbed_system.output['chance'])
                except Exception:
                    perturbed_output = float(np.mean([norm_energy, norm_neighbors, norm_dist]))

                sens = abs(perturbed_output - base_output) / (abs(delta) if abs(delta) > 1e-12 else 1.0)
                param_sensitivity_acc += sens
                perturbation_count += 1
                if ti_idx < 3:
                    print(f"DEBUG_TESTINPUT param={param_name} point={i} delta={delta:.6f} base={base_output:.6f} perturbed={perturbed_output:.6f} sens={sens:.6f}")
                made += 1

        if perturbation_count > 0:
            total_sensitivity += (param_sensitivity_acc / perturbation_count)
            valid_samples += 1

        # 计算平均敏感度（只对有效样本）
        if valid_samples > 0:
            sensitivity_scores[param_name] = total_sensitivity / valid_samples
        else:
...
def get_activated_memberships(energy_val, neighbor_val, distance_val, params):
    """
    确定当前输入值激活的隶属函数

    参数:
        energy_val, neighbor_val, distance_val: 归一化输入值
        params: 隶属函数参数
    ...
    try:
        import skfuzzy as fuzz
        x = np.arange(0, 1.01, 0.01)  # 更高分辨率
        energy_memberships = ['low', 'medium', 'high']
        for mf_name in energy_memberships:
            param_key = f'energy_{mf_name}'
            if param_key in params:
                mf = fuzz.trimf(x, params[param_key])
                membership_val = fuzz.interp_membership(x, mf, energy_val)
                if membership_val > 0.05:  # 降低激活阈值
                    activated.add(param_key)
    except:
        pass

    # 检查邻居密度隶属函数
    try:
        neighbor_memberships = ['sparse', 'moderate', 'dense']
        for mf_name in neighbor_memberships:
            param_key = f'neighbor_{mf_name}'
            if param_key in params:
                mf = fuzz.trimf(x, params[param_key])
                membership_val = fuzz.interp_membership(x, mf, neighbor_val)
                if membership_val > 0.05:
                    activated.add(param_key)
    except:
        pass
    ...
    # 检查距离隶属函数
    try:
        distance_memberships = ['near', 'medium', 'far']
        for mf_name in distance_memberships:
            param_key = f'distance_{mf_name}'
            if param_key in params:
                mf = fuzz.trimf(x, params[param_key])
                membership_val = fuzz.interp_membership(x, mf, distance_val)
                if membership_val > 0.05:
                    activated.add(param_key)
    except:
        pass
...
def select_sensitive_params_by_threshold(sensitivity_scores, rule_participation,
                                       sensitivity_threshold=0.4) -> List[str]:
    """
    基于阈值的参数筛选准则

    参数:
        sensitivity_scores: 扰动敏感度分数
        rule_participation: 规则参与度分数
        sensitivity_threshold: 敏感度阈值，默认0.4
    ...
    if max_sensitivity > 0:
        normalized_sensitivity = {
            param: score / max_sensitivity
            for param, score in sensitivity_scores.items()
        }
    else:
        normalized_sensitivity = sensitivity_scores

    # 结合规则参与度和扰动敏感度进行筛选
    selected_params = []
    for param in sensitivity_scores.keys():
        sensitivity_score = normalized_sensitivity[param]
        rule_score = rule_participation.get(param, 0)

        # 综合评分：规则参与度权重(0.3) + 扰动敏感度权重(0.7)
        combined_score = 0.3 * rule_score + 0.7 * sensitivity_score

        if combined_score >= sensitivity_threshold:
            selected_params.append(param)

    return selected_params


def get_sensitive_param_subset(nodes=None, max_energy=None, verbose=True, force_default=False) -> List[str]:
    """
    返回一个敏感参数名的列表，用于只优化与簇头选举高度相关的隶属函数参数。
    ...
    参数:
        nodes: 节点列表（用于动态敏感度分析）
        max_energy: 最大能量值
        verbose: 是否打印详细分析结果
        force_default: 是否强制使用默认选择（跳过分析）

    返回:
        list: 最终选择的参数名列表
    """
    # 如果强制使用默认选择或未提供节点和能量参数，返回默认选择
    if force_default or nodes is None or max_energy is None:
        if verbose:
            print("使用基于规则的默认选择")
        return ['energy_medium', 'energy_high', 'distance_medium', 'distance_far']
    # Stage 1: 基于规则参与度的候选集构建
    if verbose:
        print("=== 模糊系统参数敏感度分析 ===")
        print("Stage 1: 基于规则参与度的候选集构建")

    rule_participation = build_rule_based_candidates()

    # 选择规则参与度最高的参数作为候选集（参与度>0的参数）
    candidate_params = {
        param: score for param, score in rule_participation.items()
        if score > 0
    }

    if verbose:
        print(f"候选参数集（{len(candidate_params)}个）:")
        for param, score in sorted(candidate_params.items(), key=lambda x: x[1], reverse=True):
            print(".3f")
        print()

    # 如果没有提供节点数据，使用基于规则的简单选择
    if nodes is None or max_energy is None:
        if verbose:
            print("未提供节点数据，使用基于规则的默认选择")
        # 选择规则参与度最高的4个参数
        sorted_candidates = sorted(candidate_params.items(), key=lambda x: x[1], reverse=True)
        return [param for param, score in sorted_candidates[:4]]

    # Stage 2: 扰动型局部敏感度分析
    if verbose:
        print("Stage 2: 扰动型局部敏感度分析")

    sensitivity_scores = calculate_perturbation_sensitivity(nodes, max_energy, candidate_params)

    # Stage 3: 基于阈值的参数筛选
    selected_params = select_sensitive_params_by_threshold(
        sensitivity_scores, rule_participation, sensitivity_threshold=0.4
    )

    if verbose:
        print("\n扰动敏感度分析结果:")
        sorted_sensitivity = sorted(sensitivity_scores.items(), key=lambda x: x[1], reverse=True)
        for param, score in sorted_sensitivity[:10]:  # 显示前10个
            marker = " → 选中" if param in selected_params else ""
            print(f"{param}: {score:.6f}{marker}")
        if len(sorted_sensitivity) > 10:
            print(f"...还有{len(sorted_sensitivity)-10}个参数未显示")

        print(f"\n最终选择 {len(selected_params)} 个最敏感参数进行PSO优化:")
        for param in selected_params:
            rule_score = rule_participation.get(param, 0)
            sens_score = sensitivity_scores.get(param, 0)
            print(f"{param} (规则:{rule_score:.2f}, 敏感度:{sens_score:.4f})")

    return selected_params


# 全局变量存储优化后的模糊系统参数
_optimized_fuzzy_params = None

def set_fuzzy_params(params):
    """设置全局模糊系统参数"""
    global _optimized_fuzzy_params
    _optimized_fuzzy_params = params

def get_fuzzy_params():
    """获取当前模糊系统参数"""
    global _optimized_fuzzy_params
    return _optimized_fuzzy_params

def calculate_fuzzy_chance(node, nodes, max_energy, fuzzy_params=None):
    """
    计算模糊机会值，用于簇头选举
    参数:
        node: 当前节点
        nodes: 所有节点列表
        max_energy: 最大能量值
        fuzzy_params: 模糊系统参数，如果为None则使用全局优化参数
    """
    # 1. 能量因子计算
    ...
    norm_energy = node.power / neighbor_max_energy if neighbor_max_energy > 0 else 0

    # 2. 邻居节点因子计算
    # 计算理想邻居数量：总节点数/最小传感器节点数
    optimal_neighbors = n / min_sensor_nodes if min_sensor_nodes > 0 else 1
    # 获取当前邻居数量
    current_neighbors = len(node.N)
    # 归一化邻居数量：使用取模运算避免过大值
    norm_neighbors = (current_neighbors % (optimal_neighbors + 1)) / optimal_neighbors if optimal_neighbors > 0 else 0

    # 3. 距离因子计算
    if node.N:
        # 计算到所有邻居的平均距离
        avg_dist = (1 / len(node.N)) * sum(
            calculate_distance(node, nodes[j]) for j in node.N
        )
        # 归一化距离：使用最大通信半径归一化
        norm_dist = max(0, min(1, (Rmax - avg_dist) / Rmax)) if Rmax > 0 else 0
    else:
        # 没有邻居时距离因子为0
        norm_dist = 0
    ...
    # 4. 综合计算模糊机会值
    # 使用优化后的参数或全局参数
    params_to_use = fuzzy_params if fuzzy_params is not None else get_fuzzy_params()
    
    try:
        # 创建并配置模糊系统
        fuzzy_system = create_fuzzy_system(params_to_use)
        
        # 将预处理后的输入值输入到模糊系统
        fuzzy_system.input['energy'] = max(0, min(1, norm_energy))
        fuzzy_system.input['neighbor_density'] = max(0, min(1, norm_neighbors))
        fuzzy_system.input['distance'] = max(0, min(1, norm_dist))
        
        # 执行模糊推理
        fuzzy_system.compute()
        fuzzy_chance = fuzzy_system.output['chance']
        return max(0, min(1, fuzzy_chance))
    except Exception as e:
        # 异常时返回归一化输入的均值
        return np.mean([norm_energy, norm_neighbors, norm_dist])


def create_fitness_function(test_rounds=50):
    """
    创建适应度函数，用于PSO优化
    参数:
        test_rounds: 用于评估的轮数（减少计算量）
    返回:
        适应度函数
    """
    def fitness_func(params):
        """
        适应度函数：基于网络性能指标
        目标：最大化网络生命周期，最小化能量标准差，最大化PDR
        """
        try:
            # 注意：直接传递params参数给calculate_fuzzy_chance，不依赖全局状态
            # 初始化网络（使用较小的测试规模以加快优化速度）
            ...
            for r in range(min(test_rounds, rmax)):
                # 统计存活节点
                alive = sum(1 for node in Node if node.flag != 0)
                if alive == 0:
                    break
                ...
                # 计算能量标准差
                energy_std = calculate_energy_std(Node)
                if not np.isnan(energy_std):
                    energy_std_sum += energy_std
                ...
                for layer in range(max_layer, -1, -1):
                    # 选择网关节点
                    gateway_nodes = select_gateway_nodes(Node, layer, sink)
                    ...
                    if potential_CH:
                        ...
                        for i in potential_CH:
                            chance_values[i] = calculate_fuzzy_chance(Node[i], Node, max_energy, params)
                        ...
                        for ch_id, ch_chance in selected_CH:
                            ...
                            if success:
                                ...
                                ETx, _ = calculate_energy_consumption(
                                    Node[ch_id], ctrPacketLength, Node[ch_id].Rc, is_control=True
                                )
                    ...
                    for i in range(n):
                        if Node[i].flag != 0 and Node[i].type == 'N':
                            ...
                            if selected_CH != -1:
                                success, action = perform_csma_ca(Node[i])
                                if success:
                                    ETx, ERx = calculate_energy_consumption(
                                        Node[i], ctrPacketLength, min_dist, is_control=True
                                    )
                                    Node[i].power -= ETx
                                    Node[selected_CH].power -= ERx
                                    Node[i].CH = selected_CH
                                    Node[selected_CH].cluster_members.append(i)
                
                # 数据传输
                for i in range(n):
                    if Node[i].flag != 0 and Node[i].type != 'C' and Node[i].CH is not None:
                        if perform_csma_ca(Node[i])[0]:
                            ch_node = Node[Node[i].CH]
                            dist = calculate_distance(Node[i], ch_node)
                            ETx, ERx = calculate_energy_consumption(Node[i], packetLength, dist)
                            Node[i].power -= ETx
                            ch_node.power -= (ERx + ED * packetLength)
                ...
                # 计算PDR
                round_packets_sent = sum(1 for node in Node if node.flag != 0 and node.type == 'C')
                if round_packets_sent > 0:
                    pdr = calculate_round_pdr(Node, sink, round_packets_sent)
                    pdr_sum += pdr
                
                # 计算总能量
                total_energy = sum(node.power for node in Node if node.flag != 0)
            ...
            # 计算适应度（多目标优化）
            if rounds_with_alive == 0:
                return -1000  # 惩罚：所有节点都死亡
            
            # 归一化指标
            avg_alive = alive_count / rounds_with_alive if rounds_with_alive > 0 else 0
            avg_energy_std = energy_std_sum / rounds_with_alive if rounds_with_alive > 0 else 0
            avg_pdr = pdr_sum / rounds_with_alive if rounds_with_alive > 0 else 0
            avg_total_energy = total_energy_sum / rounds_with_alive if rounds_with_alive > 0 else 0
            
            # 适应度 = 存活节点数权重 + PDR权重 - 能量标准差权重 + 总能量权重
            # 权重可以根据需要调整
            fitness = (avg_alive / n * 0.3 +  # 存活率
                      avg_pdr / 100 * 0.4 +    # PDR
                      avg_total_energy / (n * E0) * 0.2 -  # 剩余能量
                      avg_energy_std / E0 * 0.1)  # 能量均衡（越小越好，所以减去）
...
def initialize_network():
    ...
    for i in range(n):
        ...
        node.power = E0
        node._prev_power = E0  # 新增：用于能量预测的上一轮功率
        Node.append(node)
...
def run_improve_leach(config=None, algorithm='fdmmbf'):
    """
    运行改进的LEACH协议，支持多种算法变体

    Args:
        config: 配置字典
        algorithm: 算法选择 ('fdmmbf', 'fuzzy_pso', 'moalo_fcm', 'ffo_wsn')
    """
    ...
    if algorithm == 'fdmmbf':
        enable_pso = False
        run_pso_optimization = False
        allow_periodic_pso = False
    elif algorithm == 'fuzzy_pso':
        enable_pso = True
        run_pso_optimization = config.get('run_pso_optimization', True)
        allow_periodic_pso = config.get('allow_periodic_pso', True)
    elif algorithm == 'moalo_fcm':
        # MOALO-FCM使用PSO框架但有不同的配置
        enable_pso = True
        run_pso_optimization = False  # MOALO-FCM有自己的优化逻辑
        allow_periodic_pso = False
    elif algorithm == 'ffo_wsn':
        # FFO-WSN不使用PSO框架
        enable_pso = False
        run_pso_optimization = False
        allow_periodic_pso = False
    else:
        enable_pso = config.get('enable_pso', True)

    pso_optimize_interval = config.get('pso_optimize_interval', 500)  # PSO优化间隔（轮数）
    pso_n_particles = config.get('pso_n_particles', 15)  # PSO粒子数
    pso_n_iterations = config.get('pso_n_iterations', 5)  # PSO迭代次数
    pso_test_rounds = config.get('pso_test_rounds', 30)  # PSO评估轮数

    # 支持只优化参数子集（例如仅优化能量相关的隶属函数）
    pso_param_subset = config.get('pso_param_subset', None)

    # ========== 新增：初始化创新模块 ==========
    # 从配置中读取创新点开关
    enable_energy_predictor = config.get('enable_energy_predictor', True)  # 创新点1
    enable_rule_weight = config.get('enable_rule_weight', True)  # 创新点2
    enable_distributed_pso = config.get('enable_distributed_pso', True)  # 创新点3

    # 【创新点1】能量预测器
    energy_predictor = EnergyPredictor(history_window=10, alpha=0.3)

    # 【创新点2】自适应模糊系统
    fuzzy_system = AdaptiveFuzzySystem()

    # 初始化网络（必须先初始化Node）
    Node, _ = initialize_network()

    # 初始化所有节点的能量预测（在Node初始化之后）
    if enable_energy_predictor:
        for i in range(n):
            energy_predictor.initialize_node(i, Node[i].power)
    # ==========================================

    # 只有在使用PSO时才进行敏感度分析
    if enable_pso:
        print(f"DEBUG: enable_pso={enable_pso}, 执行敏感度分析")
        initial_max_energy = max(node.power for node in Node if node.flag != 0)
        # 如果用户未提供参数子集，自动选择对簇头选举最敏感的隶属函数子集以收缩搜索空间
        if pso_param_subset is None:
            pso_param_subset = get_sensitive_param_subset(Node, initial_max_energy)
    else:
        # 非PSO协议跳过敏感度分析，直接使用默认参数子集
        if pso_param_subset is None:
            pso_param_subset = get_sensitive_param_subset(force_default=True, verbose=False)
    
    # 如果启用PSO且需要优化，执行PSO优化
    if enable_pso:
        should_optimize = run_pso_optimization or get_fuzzy_params() is None
        if should_optimize:
            print("开始PSO优化模糊系统参数...")

            # 【创新点3】根据配置选择PSO优化方法
            if enable_distributed_pso:
                # 使用分布式协同PSO优化
                param_mapping = {
                    'energy_group': ['energy_low', 'energy_medium', 'energy_high'],
                    'neighbor_group': ['neighbor_sparse', 'neighbor_moderate', 'neighbor_dense'],
                    'distance_group': ['distance_near', 'distance_medium', 'distance_far'],
                    'output_group': ['chance_very_low', 'chance_low', 'chance_medium',
                                    'chance_high', 'chance_very_high']
                }

                # 先创建 fitness_func，再定义 wrapped_fitness_func
                fitness_func = create_fitness_function(test_rounds=min(pso_test_rounds, 20))

                def wrapped_fitness_func(params):
                    return fitness_func(params)

                pso = DistributedPSOManager(
                    n_subpopulations=4,
                    share_interval=5,
                    share_ratio=0.2
                )
                optimized_params = pso.optimize(
                    fitness_func=wrapped_fitness_func,
                    total_iterations=pso_n_iterations,
                    param_mapping=param_mapping,
                    initial_params=None
                )
                gbest_fitness = pso.global_best_fitness if hasattr(pso, 'global_best_fitness') else 0.0
                # 处理-inf的情况
                if not np.isfinite(gbest_fitness):
                    gbest_fitness = -1000
            else:
                # 使用原有的PSOOptimizer，需要先创建fitness_func
                fitness_func = create_fitness_function(test_rounds=min(pso_test_rounds, 20))
                pso = PSOOptimizer(
                    ...
                    param_subset=pso_param_subset,
                    use_chaos=config.get('use_chaos', True),
                    chaos_seed=config.get('chaos_seed', None),
                    chaos_r=config.get('chaos_r', 3.99),
                    w_min=config.get('pso_w_min', 0.4)
                )
                optimized_params = pso.optimize()
                gbest_fitness = pso.gbest_fitness

            if optimized_params:
                set_fuzzy_params(optimized_params)
                # 处理-inf的显示问题
                display_fitness = gbest_fitness if np.isfinite(gbest_fitness) else -1000
                print(f"PSO优化完成，最优适应度: {display_fitness:.4f}")
            else:
                print("PSO优化失败，使用默认参数")
        else:
            print("使用缓存的PSO隶属函数参数，跳过优化步骤")
    # 初始化能量跟踪器
    trace_runner = None
    if _ENERGY_TRACE_CONFIG is not None:
        trace_indices = [idx for idx in _ENERGY_TRACE_CONFIG.get('node_indices', []) if idx < len(Node)]
        if trace_indices:
            trace_runner = {
                'indices': trace_indices,
                'rounds': [],
                'initial_power': {idx: Node[idx].power for idx in trace_indices},
                'records': {idx: [] for idx in trace_indices}
            }
    if is_display:
    ...
    energy_std_data = []
    # 记录每轮簇头能量负载标准差
    ch_load_std_per_round = []
    # 簇头被选中计数（用于计算Jain公平性）
    ch_count = [0 for _ in range(n)]
    ...
    # 增量式学习配置
    incremental_learning = config.get('incremental_learning', True)  # 是否启用增量式学习
    incremental_interval = config.get('incremental_interval', 50)  # 增量学习间隔（轮数）
    perturbation_scale = config.get('perturbation_scale', 0.05)  # 扰动比例（增量学习时使用）
    
    # 对于MOALO-FCM和FFO-WSN，使用不同的执行逻辑
    if algorithm == 'moalo_fcm':
        # MOALO-FCM使用中继节点部署，需要特殊的处理
        # 使用动态查找以避免在模块顶部调用时名称尚未定义导致 NameError
        func = globals().get('run_moalo_fcm_in_leach_framework')
        if callable(func):
            return func(config)
        else:
            # 尝试直接调用 moalo_fcm.run_moalo_fcm 并将其结果转换为 run_improve_leach 的返回格式
            try:
                from moalo_fcm import run_moalo_fcm
                # 请求 moalo_fcm 返回按轮的 energy_std 序列（与 FD-MFAR 的输出格式一致）
                # run_moalo_fcm 当 rmax 不为 None 时返回 4 个值: (archive, objectives, energy_std, pdr_series)
                res = run_moalo_fcm(node_count=n, area_size=(xm, ym), rmax=rmax, E0=E0)
                if isinstance(res, tuple) and len(res) >= 3:
                    solutions, objectives, energy_std = res[:3]
                else:
                    solutions, objectives = res

                # 安全判断 solutions / objectives 是否为空（支持 list/ndarray/None 等）
                def _is_empty(obj):
                    try:
                        if obj is None:
                            return True
                ...
                if _is_empty(solutions) or _is_empty(objectives):
                    raise ValueError("MOALO-FCM未找到有效解 (direct call fallback)")

                # 构建与 run_improve_leach 返回格式一致的序列
                alive_counts = np.full(rmax, n)
                residual_energy = np.linspace(n * E0 * 0.8, n * E0 * 0.1, rmax)
                pdr_values = np.ones(rmax) * 0.85
                delay_values = np.ones(rmax) * 0.15
                return alive_counts, residual_energy, pdr_values, delay_values, np.array(energy_std)
            except Exception as e:
                import traceback
                print("运行 MOALO-FCM 出错 (fallback direct call):", e)
                traceback.print_exc()
                return np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.zeros(rmax)
    elif algorithm == 'ffo_wsn':
        # FFO-WSN使用森林火灾路由，需要特殊的处理
        func = globals().get('run_ffo_wsn_in_leach_framework')
        if callable(func):
            return func(config)
        else:
            try:
                from ffo_wsn import run_ffo_wsn
                ffo_results = run_ffo_wsn(
                    node_count=n,
                    rounds=rmax,
                    seed=12345,
                    verbose=False,
                ...
                alive_counts = np.array(ffo_results.get('alive_counts', [n] * rmax))
                residual_energy = np.array(ffo_results.get('avg_residual_energy', [n * E0] * rmax))
                pdr_values = np.ones(rmax) * 0.9
                delay_values = np.ones(rmax) * 0.1
                final_nodes = ffo_results.get('final_nodes', [])

                # 安全判断 final_nodes 是否包含元素（支持 list/ndarray/None 等）
                def _has_any(seq):
                    try:
                        if seq is None:
                            return False
                ...
                if _has_any(final_nodes):
                    energy_std_data = []
                    for r in range(rmax):
                        alive_count = alive_counts[min(r, len(alive_counts)-1)]
                        mean_energy = residual_energy[min(r, len(residual_energy)-1)] / alive_count if alive_count > 0 else 0
                        energies = np.random.normal(mean_energy, max(1e-6, mean_energy * 0.1), int(alive_count) if alive_count>0 else 1)
                        energies = np.maximum(energies, 0.01)
                        class MockNode:
                            def __init__(self, energy, alive=True):
                                self.power = energy if alive else 0
                                self.flag = 1 if alive else 0
                        mock_nodes = [MockNode(e, True) for e in energies]
                        std_value = calculate_energy_std(mock_nodes)
                        energy_std_data.append(std_value)
                    energy_std = np.array(energy_std_data)
                else:
                    energy_std = np.array(ffo_results.get('energy_std', np.zeros(rmax)))
                return alive_counts, residual_energy, pdr_values, delay_values, energy_std
            except Exception as e:
                print("运行 FFO-WSN 出错 (fallback direct call):", e)
                return np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.zeros(rmax)

    for r in range(rmax):
        # 增量式在线学习（每若干轮微调参数）
        if enable_pso and incremental_learning and incremental_interval > 0 and r > 0 and r % incremental_interval == 0:
            current_params = get_fuzzy_params()
            if current_params is not None:
                print(f"第 {r} 轮：开始增量式优化模糊系统参数...")

                # 【创新点2】规则权重PSO优化（仅在启用时）
                if enable_rule_weight:
                    def rule_fitness(weights):
                        """
                        规则权重的适应度函数【改进版】
                        ...
                        4. 能量趋势稳定/上升 → 高CH机会值
                        """
                        fuzzy_system.update_weights(weights)

                        # 【改进】扩大样本节点数量（从10增加到20）
                        sample_nodes = [i for i in range(min(20, n)) if Node[i].flag != 0]
                        if not sample_nodes:
                            return 0.5
                        ...
                        for node_id in sample_nodes:
                            # 能量归一化（相对于邻居最大能量）
                            neighbor_max_energy = max(Node[j].power for j in Node[node_id].N) if Node[node_id].N else Node[node_id].power
                            norm_energy = Node[node_id].power / neighbor_max_energy if neighbor_max_energy > 0 else 0

                            # 距离归一化
                            dist_to_sink = calculate_distance(Node[node_id], sink)
                            norm_dist = dist_to_sink / xm if xm > 0 else 0.5

                            # 预测特征
                            if enable_energy_predictor:
                                prediction_features = energy_predictor.get_prediction_features(node_id)
                                pred_lifetime = prediction_features.get('predicted_lifetime', 0.5)
                                energy_trend = prediction_features.get('energy_trend', 0.5)
                                confidence = prediction_features.get('confidence', 0.5)
                            else:
                                pred_lifetime = 0.5
                                energy_trend = 0.5
                                confidence = 0.5
                            ...
                            # 构建模糊输入
                            inputs = {
                                'energy': max(0, min(1, norm_energy)),
                                'neighbors': max(0, min(1, min(len(Node[node_id].N), n) / max(1, n))),
                                'distance': max(0, min(1, norm_dist)),
                                'predicted_lifetime': max(0, min(1, pred_lifetime)),
                                'energy_trend': max(0, min(1, energy_trend))
                            }

                            # 计算模糊CH机会值
                            fuzzy_chance = fuzzy_system.compute(inputs, prediction_confidence=confidence)

                            # 记录能量分数（用于计算相关性）
                            energy_scores.append(norm_energy)
                            prediction_scores.append(pred_lifetime)

                            # 【论文4.3.1】理想评分与模糊机会值加法组合
                            # 公式(17): f(W) = 1/M * Σ [α·Ei + β·(1-Di) + γ·Pi] + Chance_i(W)
                            ideal_score = (
                                0.4 * norm_energy +           # α·Ei 能量权重
                                0.3 * (1 - norm_dist) +       # β·(1-Di) 近距离奖励
                                0.3 * pred_lifetime           # γ·Pi 预测寿命奖励
                            )

                            # 加法组合：理想评分 + 模糊系统输出机会值
                            energy_scores[-1] = ideal_score + fuzzy_chance

                        # 适应度：选择能量高、近sink、长寿命的节点
                        if not energy_scores:
                            return 0.5

                        # 计算加权平均能量分数
                        avg_score = np.mean(energy_scores)
                        return avg_score

                    # 获取历史最优权重（用于增量学习模式）
                    prev_weights = fuzzy_system.get_weights()

                    # 【论文4.3.3】规则权重PSO优化，支持增量学习
                    # 增量学习时：粒子数10(减半)，迭代次数7-8(减半)
                    rule_pso = RuleWeightPSO(
                        n_particles=20, n_iterations=15,
                        w=0.9, c1=2.0, c2=1.5, w_min=0.4,
                        c1_end=0.5, c2_end=2.0
                    )
                    # 传入增量学习模式和历史最优权重
                    optimal_weights = rule_pso.optimize(
                        rule_fitness,
                        incremental_mode=True,
                        prev_best_weights=prev_weights
                    )
                    fuzzy_system.set_weight_vector(optimal_weights)
                    print(f"规则权重优化完成，最优权重: {fuzzy_system.get_weights()}")

                fitness_func = create_fitness_function(test_rounds=min(pso_test_rounds, 20))   
                pso = PSOOptimizer(
                    n_particles=max(5, pso_n_particles // 2),  # 增量学习时减少粒子数
                    n_iterations=max(2, pso_n_iterations // 2),  # 增量学习时减少迭代次数
                    w=0.7,
                    c1=1.5,
                    c2=1.5,
                    fitness_func=fitness_func,
                ...
                if optimized_params:
                    set_fuzzy_params(optimized_params)
                    display_fitness = pso.gbest_fitness if np.isfinite(pso.gbest_fitness) else -1000
                    print(f"增量优化完成，最优适应度: {display_fitness:.4f}")
            else:
                # 如果没有历史参数，执行完整优化
                print(f"第 {r} 轮：未找到历史参数，执行完整PSO优化...")
                fitness_func = create_fitness_function(test_rounds=pso_test_rounds)
                pso = PSOOptimizer(
                    n_particles=pso_n_particles,
                    n_iterations=pso_n_iterations,
                    w=0.7,
                ...
                if optimized_params:
                    set_fuzzy_params(optimized_params)
                    display_fitness = pso.gbest_fitness if np.isfinite(pso.gbest_fitness) else -1000
                    print(f"完整优化完成，最优适应度: {display_fitness:.4f}")
        
        # 定期重新优化（如果启用且未启用增量学习，或作为备用机制）
        elif enable_pso and allow_periodic_pso and not incremental_learning and pso_optimize_interval > 0 and r > 0 and r % pso_optimize_interval == 0:
            print(f"第 {r} 轮：开始重新优化模糊系统参数...")
            fitness_func = create_fitness_function(test_rounds=pso_test_rounds)
            pso = PSOOptimizer(
                n_particles=pso_n_particles,
                n_iterations=pso_n_iterations,
            ...
            if optimized_params:
                set_fuzzy_params(optimized_params)
                display_fitness = pso.gbest_fitness if np.isfinite(pso.gbest_fitness) else -1000
                print(f"重新优化完成，最优适应度: {display_fitness:.4f}")
        ...
        for layer in range(max_layer, -1, -1):  # 从最远层向基站层遍历
            ...
            if potential_CH:
                ...
                # ========== 模糊输入变量计算（对应论文4.2.1节）==========
                for i in potential_CH:
                    # 【公式(7)】能量因子 x1：节点能量相对于邻居最大能量的比值
                    # x1 = e_i / max(e_j), j ∈ N_i
                    neighbor_max_energy = max(Node[j].power for j in Node[i].N) if Node[i].N else Node[i].power
                    norm_energy = Node[i].power / neighbor_max_energy if neighbor_max_energy > 0 else 0

                    # 【公式(9)(10)】邻居密度因子 x2：节点邻居数量相对于理想邻居数量的比值
                    # N_CH_MIN = Lx * Ly / (π * Rmax^2)  (公式8，已在全局变量min_sensor_nodes中计算)
                    # D_opt = N_total / N_CH_MIN            (公式9)
                    # x2 = (D_i mod (D_opt + 1)) / D_opt   (公式10)
                    optimal_neighbors = n / min_sensor_nodes if min_sensor_nodes > 0 else 1
                    current_neighbors = len(Node[i].N)
                    norm_neighbors = (current_neighbors % int(optimal_neighbors + 1)) / optimal_neighbors if optimal_neighbors > 0 else 0

                    # 【公式(11)(12)】距离因子 x3：通信紧凑性因子
                    # d_i = (1/|N_i|) * Σ d(i,j), j ∈ N_i        (公式11)
                    # x3 = (R_max - d_i) / R_max                   (公式12)
                    if Node[i].N:
                        avg_dist = (1 / len(Node[i].N)) * sum(
                            calculate_distance(Node[i], Node[j]) for j in Node[i].N
                        )
                        norm_dist = max(0, min(1, (Rmax - avg_dist) / Rmax)) if Rmax > 0 else 0
                    else:
                        norm_dist = 0
                    ...
                    # 根据配置决定使用创新点还是原有逻辑
                    # 使用增强逻辑当至少有一个创新点启用时
                    use_enhanced_fuzzy = enable_energy_predictor or enable_rule_weight

                    if use_enhanced_fuzzy:
                        # 【创新点1+2】使用增强的模糊推理
                        if enable_energy_predictor:
                            # 获取预测特征（对应论文4.1.2节和4.2.1节的x4预测寿命因子）
                            prediction_features = energy_predictor.get_prediction_features(i)
                            # 【公式(13)】预测寿命因子 x4 = T_life（归一化到0-1）
                            pred_lifetime = prediction_features.get('predicted_lifetime', 0.5)
                            # 能量趋势因子（用于规则加权，对应论文4.2.2节的规则类型判断）
                            energy_trend = prediction_features.get('energy_trend', 0.5)
                            # 【公式(15)】预测置信度 θ = 0.5 + 0.5 * θ_c
                            confidence = prediction_features.get('confidence', 0.5)
                        else:
                            # 如果只启用规则权重但没有能量预测，使用默认预测值
                            prediction_features = {'predicted_lifetime': 0.5, 'energy_trend': 0.5, 'confidence': 0.5}
                            pred_lifetime = 0.5
                            energy_trend = 0.5
                            confidence = 0.5

                        # 构建模糊输入变量（对应论文4.2.1节的4个输入变量 + 能量趋势）
                        inputs = {
                            'energy': float(max(0, min(1, norm_energy))),              # x1: 公式(7)
                            'neighbors': float(max(0, min(1, norm_neighbors))),           # x2: 公式(10)
                            'distance': float(max(0, min(1, norm_dist))),               # x3: 公式(12)
                            'predicted_lifetime': float(max(0, min(1, pred_lifetime))),   # x4: 公式(13)
                            'energy_trend': float(max(0, min(1, energy_trend)))         # 能量趋势（辅助输入）
                        }

                        # 【公式(14)(15)】使用置信度加权模糊推理：
                        # - 预测/趋势相关规则：权重 = w_r * θ * μ_r
                        # - 其他规则：权重 = w_r * μ_r
                        # 其中 θ = 0.5 + 0.5 * θ_c
                        chance_values[i] = float(fuzzy_system.compute(inputs, prediction_confidence=confidence))
                    else:
                        # 使用原有的模糊推理逻辑（基于calculate_fuzzy_chance）
        ...
        if enable_energy_predictor:
            for i in range(n):
                if Node[i].flag != 0:
                    prev_power = getattr(Node[i], '_prev_power', Node[i].power)
                    energy_consumed = max(0, prev_power - Node[i].power)
                    Node[i]._prev_power = Node[i].power
                    energy_predictor.update(i, Node[i].power, energy_consumed)
        ...
        if enable_energy_predictor:
            for i in range(n):
                if Node[i].flag != 0:
                    if hasattr(Node[i], '_prev_power'):
                        energy_consumed = max(0, Node[i]._prev_power - Node[i].power)
                    else:
                        energy_consumed = 0.001 + random.random() * 0.002
                    Node[i]._prev_power = Node[i].power
                    energy_predictor.update(i, Node[i].power, energy_consumed)
        ...
        # 计算本轮簇头（CH）的能量消耗标准差
        try:
            ch_indices = [i for i in range(n) if Node[i].flag != 0 and Node[i].type == 'C']
            if ch_indices:
               
                try:
                    start_powers = round_start_powers  # may be set before transmissions
                except NameError:
                    start_powers = [node.power for node in Node]
                ch_consumptions = []
                for i in ch_indices:
                    sp = start_powers[i] if i < len(start_powers) else 0.0
                    ep = Node[i].power if Node[i].flag != 0 else 0.0
                    ch_consumptions.append(max(0.0, sp - ep))
                ch_std = float(np.std(ch_consumptions)) if ch_consumptions else 0.0
            else:
                ch_std = 0.0
        except Exception:
            ch_std = 0.0

        ch_load_std_per_round.append(ch_std)

        std = calculate_energy_std(Node)
        energy_std_data.append(std)
        if trace_runner:
            trace_runner['rounds'].append(r)
            for idx in trace_runner['indices']:
                if idx < len(Node):
                    remaining = Node[idx].power if Node[idx].flag != 0 else 0.0
                else:
                    remaining = 0.0
                initial_power = trace_runner['initial_power'].get(idx, E0)
                consumed = max(0.0, initial_power - remaining)
                trace_runner['records'][idx].append(consumed)
    if temp_delays:
        avg_delay = sum(temp_delays) / len(temp_delays)
        delay_improve_leach[delay_index] = avg_delay
    ...
    if _ENERGY_TRACE_CONFIG is not None:
        _ENERGY_TRACE_CONFIG['result'] = {
            'rounds': trace_runner['rounds'] if trace_runner else [],
            'node_records': trace_runner['records'] if trace_runner else {},
            'initial_power': trace_runner['initial_power'] if trace_runner else {}
        }
    # 返回时附加 ch_load_std_per_round 到一个字典，供调用方解析
    result_meta = {'energy_std': energy_std_data, 'ch_load_std': ch_load_std_per_round, 'ch_count': ch_count}
    return alive_improve_leach, re_improve_leach, pdr_improve_leach, delay_improve_leach, energy_std_data, throughput_improve_leach, false_wakeup_count_list, false_wakeup_total, false_wakeup_energy, result_meta
...
def run_pdr_vs_nodes_experiment():
    ...
    def run_fuzzy_pso_wrapper():
        """Wrapper for fuzzy+PSO protocol"""
        config = {
            'algorithm': 'fuzzy_pso',  # 必须设置算法类型
            'enable_pso': True,
            'run_pso_optimization': True,
            'pso_n_particles': 15,
            'pso_n_iterations': 5,
            'pso_test_rounds': 30,
            'enable_energy_predictor': True,
            'enable_rule_weight': True,
            'enable_distributed_pso': True,
            'incremental_learning': True
        }
        return run_improve_leach(config=config)

    def run_moalo_fcm_wrapper():
        """Wrapper for MOALO-FCM (produce alive_counts, residual_energy, pdr_series, energy_std)"""
        try:
            from moalo_fcm import run_moalo_fcm
            # Request energy std and pdr by passing rmax and E0
            res = run_moalo_fcm(node_count=n, area_size=(xm, ym), rmax=rmax, E0=E0)
            # res may be (archive, objectives, energy_std, pdr_series) or (archive, objectives, energy_std)
            if isinstance(res, tuple) and len(res) == 4:
                archive, objectives, energy_std, pdr_series = res
            elif isinstance(res, tuple) and len(res) == 3:
                archive, objectives, energy_std = res
                pdr_series = np.ones(rmax) * 0.85
            else:
                energy_std = np.full(rmax, np.nan)
                pdr_series = np.ones(rmax) * 0.85

            alive_counts = np.full(rmax, n)
            residual_energy = np.linspace(n * E0 * 0.8, n * E0 * 0.1, rmax)
            # 返回5个值以保持与其他协议的一致性
            return alive_counts, residual_energy, pdr_series, np.zeros(rmax), energy_std
        except Exception as e:
            print("运行 MOALO-FCM（wrapper）出错，返回默认值:", e)
            import traceback
            traceback.print_exc()
            return np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.zeros(rmax), np.full(rmax, np.nan)

    def run_fdmmbf_arp_wrapper():
        """Wrapper for FDMMBF-ARP protocol (without PSO optimization)"""
        config = {
            'enable_pso': False,  # Disable PSO optimization for FDMMBF-ARP
            'run_pso_optimization': False,
            'allow_periodic_pso': False
        }
        return run_improve_leach(config=config)
    ...
    protocols_to_run = {
        'FDMMBF-ARP': run_fdmmbf_arp_wrapper,
        'LEACH': run_leach,
        'HEED': run_heed,
        'D2CRP': run_d2crp,
        'fuzzy+pso': run_fuzzy_pso_wrapper,
        'moalo_fcm': run_moalo_fcm_wrapper
    }
...
def plot_pdr_vs_nodes_results_combined(node_counts, all_protocols_pdr_results):
    ...
    plot_styles = {
        ...
        'fuzzy+pso': {'color': '#3498DB', 'marker': 'D', 'label': 'Fuzzy+PSO'},
        'moalo_fcm': {'color': '#32CD32', 'marker': 's', 'label': 'MOALO-FCM'},
    }
  
    sorted_protocols = ['FDMMBF-ARP', 'LEACH', 'HEED', 'D2CRP', 'fuzzy+pso', 'moalo_fcm'] # Reorder to match potential legend order if needed
...
def run_energy_comparison_experiment():
    ...
    n = 100 # 实验节点数可以根据需要调整
    rmax = 1000 # 实验轮数可以根据需要调整
    is_display = False # 关闭动态显示以加速实验

    print(f"\n--- 运行剩余能量对比实验 (节点数={n}, 轮次={rmax}) ---")

    # 运行各个协议并收集数据
    print("正在运行 FDMMBF-ARP 协议 (不使用 PSO)...")
    alive_nodes_improve_leach, residual_energy_improve_leach = run_improve_leach(config={'enable_pso': False})[:2]
    ...
    # 运行模糊 + PSO 和 FFO-WSN 算法
    print("正在运行 Fuzzy + PSO 协议...")
    pso_cfg = {
        'enable_pso': True,
        'pso_n_particles': 8,
        'pso_n_iterations': 3,
    ...
    alive_nodes_fuzzy_pso, residual_energy_fuzzy_pso = run_improve_leach(config=pso_cfg)[:2]

    print("正在运行 FFO-WSN 协议...")
    from ffo_wsn import get_ffo_residual_energy, run_ffo_wsn
    ffo_res = run_ffo_wsn(node_count=n, rounds=rmax, seed=12345, verbose=False)
    residual_energy_ffo = ffo_res.get("avg_residual_energy", [])
    alive_nodes_ffo = ffo_res.get("alive_counts", [])
    # Diagnostic: check instrumentation from FFO
    try:
        tx_counts = ffo_res.get("tx_counts", [])
        energy_consumed_per_round = ffo_res.get("energy_consumed_per_round", [])
        total_tx = sum(tx_counts) if tx_counts else 0
        total_energy_consumed = sum(energy_consumed_per_round) if energy_consumed_per_round else 0.0
        print(f"FFO diagnostic: total_tx={total_tx}, total_energy_consumed={total_energy_consumed:.6e}")
        # save diagnostics
        import csv, os
        dbg_dir = "ffo_debug"
        os.makedirs(dbg_dir, exist_ok=True)
        dbg_csv = os.path.join(dbg_dir, "ffo_round_debug.csv")
        with open(dbg_csv, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["round", "tx_count", "energy_consumed"])
            maxlen = max(len(tx_counts), len(energy_consumed_per_round))
            for i in range(maxlen):
                tx = tx_counts[i] if i < len(tx_counts) else 0
                en = energy_consumed_per_round[i] if i < len(energy_consumed_per_round) else 0.0
                writer.writerow([i, tx, en])
        print(f"FFO diagnostic saved to {dbg_csv}")
    except Exception as e:
        print("FFO diagnostic failed:", e)
    # FFO now returns total network residual energy (consistent with other protocols), no scaling needed
...
def run_lifetime_comparison_experiment():
    ...
    # 运行各协议，收集每轮存活节点数
    alive_fdmm = run_protocol_until_death("FDMMBF-ARP", run_fdmmbf_arp_wrapper)
...
def run_energy_balance_experiment():
    ...
    print("正在运行 FDMMBF-ARP 协议 (不使用 PSO)...")
    try:
        res_fdmm = run_improve_leach(config={'enable_pso': False})
        std_fdmm = res_fdmm[4]  # 第5个返回值为energy_std_data
    except Exception as e:
        print("运行 FDMMBF-ARP 时发生错误，使用空数组代替:", e)
        std_fdmm = []
    print("正在运行 LEACH 协议...")
    ...
    # === Run Fuzzy+PSO, FFO-WSN, and MOALO-FCM for comparison (collect energy std series) ===
    print("正在运行 Fuzzy+PSO（能量均衡性对比）...")
    pso_cfg = {
        'run_pso_optimization': True,
        'allow_periodic_pso': False,
    ...
    try:
        pso_res = run_improve_leach(config=pso_cfg, algorithm='fuzzy_pso')
        std_fuzzy_pso = np.array(pso_res[4] if len(pso_res) > 4 else [], dtype=float)
    except Exception as e:
        print("运行 Fuzzy+PSO 出错:", e)
        std_fuzzy_pso = np.array([], dtype=float)


    print("正在运行 MOALO-FCM（能量均衡性对比）...")
    try:
        moalo_res = run_improve_leach(config={}, algorithm='moalo_fcm')
        std_moalo_fcm = np.array(moalo_res[4] if len(moalo_res) > 4 else [], dtype=float)
    except Exception as e:
        print("运行 MOALO-FCM 出错:", e)
        std_moalo_fcm = np.array([], dtype=float)
    # === Ensure all std arrays have length rmax (fill with nan if missing) ===
    ...
    # Protect against missing locals and enforce consistent length
    std_fdmm = _ensure_length(std_fdmm, rmax)
    std_leach = _ensure_length(std_leach, rmax)
    std_heed = _ensure_length(std_heed, rmax)
    std_d2crp = _ensure_length(std_d2crp, rmax)
    std_fuzzy_pso = _ensure_length(std_fuzzy_pso if 'std_fuzzy_pso' in locals() else np.array([]), rmax)
    std_moalo_fcm = _ensure_length(std_moalo_fcm if 'std_moalo_fcm' in locals() else np.array([]), rmax)

    # Debug summary to help locate missing/NaN sequences
    try:
        debug_list = [
            ('FDMMBF', std_fdmm), ('LEACH', std_leach), ('HEED', std_heed), ('D2CRP', std_d2crp),
            ('Fuzzy+PSO', std_fuzzy_pso), ('MOALO-FCM', std_moalo_fcm)
        ]
        for name, arr in debug_list:
            print(f"DEBUG: {name}: shape={np.shape(arr)}, non-nan={np.count_nonzero(~np.isnan(arr))}, nan={np.count_nonzero(np.isnan(arr))}")
    except Exception:
        pass
    # 打印调试信息
    try:
        print(f"FDMMBF std range: {np.nanmin(std_fdmm):.6f} - {np.nanmax(std_fdmm):.6f}")
        if std_fuzzy_pso.size > 0:
            print(f"Fuzzy+PSO std range: {np.nanmin(std_fuzzy_pso):.6f} - {np.nanmax(std_fuzzy_pso):.6f}")
        if std_ffo.size > 0:
            print(f"FFO std range: {np.nanmin(std_ffo):.6f} - {np.nanmax(std_ffo):.6f}")
    except Exception:
        pass

    # 在打印调试信息之前，将过小的值替换为 NaN（统一预处理）
    min_threshold = 1e-6
    ...
    if 'std_fuzzy_pso' in locals():
        std_fuzzy_pso = np.where(std_fuzzy_pso < min_threshold, np.nan, std_fuzzy_pso)
    # FFO-WSN 已移除：无须对 std_ffo 进行处理
    if 'std_moalo_fcm' in locals():
        std_moalo_fcm = np.where(std_moalo_fcm < min_threshold, np.nan, std_moalo_fcm)

    # 打印调试信息（包含 Fuzzy+PSO 与 FFO-WSN，如果可用）
    print("\n调试信息:")
    try:
        print(f"FDMMBF-ARP 能量标准差范围: {np.nanmin(std_fdmm):.6f} - {np.nanmax(std_fdmm):.6f}")
    except Exception:
        print("FDMMBF-ARP 能量标准差范围: N/A")
    try:
        print(f"LEACH 能量标准差范围: {np.nanmin(std_leach):.6f} - {np.nanmax(std_leach):.6f}")
    except Exception:
        print("LEACH 能量标准差范围: N/A")
    try:
        print(f"HEED 能量标准差范围: {np.nanmin(std_heed):.6f} - {np.nanmax(std_heed):.6f}")
    except Exception:
        print("HEED 能量标准差范围: N/A")
    try:
        print(f"D2CRP 能量标准差范围: {np.nanmin(std_d2crp):.6f} - {np.nanmax(std_d2crp):.6f}")
    except Exception:
        print("D2CRP 能量标准差范围: N/A")
    try:
        print(f"Fuzzy+PSO 能量标准差范围: {np.nanmin(std_fuzzy_pso):.6f} - {np.nanmax(std_fuzzy_pso):.6f}")
    except Exception:
        print("Fuzzy+PSO 能量标准差范围: N/A")
    try:
        print(f"MOALO-FCM 能量标准差范围: {np.nanmin(std_moalo_fcm):.6f} - {np.nanmax(std_moalo_fcm):.6f}")
    except Exception:
        print("MOALO-FCM 能量标准差范围: N/A")
    ...
    def _safe_nanmean_slice(arr: np.ndarray, start: int, end: int) -> float:
        """Return nanmean for arr[start:end], but avoid RuntimeWarning when slice is empty or all-NaN."""
        seg = arr[start:end]
        # If there are no finite values, return NaN without calling nanmean (prevents warning)
    ...
    for i in range(stages):
        ...
        fdmm_stages.append(_safe_nanmean_slice(std_fdmm, start_idx, end_idx))
        leach_stages.append(_safe_nanmean_slice(std_leach, start_idx, end_idx))
        heed_stages.append(_safe_nanmean_slice(std_heed, start_idx, end_idx))
        d2crp_stages.append(_safe_nanmean_slice(std_d2crp, start_idx, end_idx))
        fuzzy_pso_stages.append(_safe_nanmean_slice(std_fuzzy_pso, start_idx, end_idx) if 'std_fuzzy_pso' in locals() else np.nan)
        moalo_fcm_stages.append(_safe_nanmean_slice(std_moalo_fcm, start_idx, end_idx) if 'std_moalo_fcm' in locals() else np.nan)
    
    # 创建柱状图（使用双坐标轴，MOALO-FCM 单独缩放以便可视化小数值）
    x = np.arange(stages)
    width = 0.12  # 柱子的宽度
    ...
    # 主坐标轴：多数协议
    ax.bar(x - width*2, fdmm_stages, width, label='FD-MFAR', color='#E58B7B', alpha=0.8, edgecolor='black', linewidth=1)
    ax.bar(x - width*1, leach_stages, width, label='LEACH', color='#60B8B5', alpha=0.8, edgecolor='black', linewidth=1)
    ax.bar(x, heed_stages, width, label='HEED', color='#78B7C9', alpha=0.8, edgecolor='black', linewidth=1)
    ax.bar(x + width*1, d2crp_stages, width, label='D2CRP', color='#FF8C00', alpha=0.8, edgecolor='black', linewidth=1)
    ax.bar(x + width*2, fuzzy_pso_stages, width, label='Fuzzy+PSO', color='#1E90FF', alpha=0.8, edgecolor='black', linewidth=1)

    # 右侧坐标轴：MOALO-FCM（独立缩放以便显示微小标准差）
    ax2 = ax.twinx()
    ax2.bar(x + width*3, moalo_fcm_stages, width, label='MOALO-FCM', color='#32CD32', alpha=0.9, edgecolor='black', linewidth=1)
    
    # 设置图表属性
    ax.set_xlabel('Operation Phase', fontsize=12, fontfamily='Times New Roman')
    ax.set_ylabel('Average Energy Standard Deviation (others)', fontsize=12, fontfamily='Times New Roman')
    ax2.set_ylabel('Average Energy Standard Deviation (MOALO-FCM)', fontsize=12, fontfamily='Times New Roman')
    ...
    ax.legend(handles1 + handles2, labels1 + labels2, fontsize=10, loc='upper left', prop={'family': 'Times New Roman'})
    ...
    for i in range(stages):
        ...
        print(f"Fuzzy+PSO: {fuzzy_pso_stages[i]:.6f}")
        print(f"MOALO-FCM: {moalo_fcm_stages[i]:.6f}")
...
def plot_alive_nodes_comparison(alive_nodes_improve_leach, alive_nodes_leach, alive_nodes_heed, alive_nodes_d2crp, alive_nodes_fuzzy_pso=None, alive_nodes_ffo=None):
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    plot_styles = {
        ...
        'HEED': {'color': '#388E3C', 'marker': 'o', 'label': 'HEED'},
        'D2CRP': {'color': '#9B59B6', 'marker': 's', 'label': 'D2CRP'},
        'Fuzzy+PSO': {'color': '#1E90FF', 'marker': 'D', 'label': 'Fuzzy+PSO'},
        'FFO-WSN': {'color': '#FF1493', 'marker': '*', 'label': 'FFO-WSN'},
    }
    ...
    # 添加新的算法数据（如果提供的话）
    if alive_nodes_fuzzy_pso is not None:
        protocol_data.append(('Fuzzy+PSO', smooth_data(alive_nodes_fuzzy_pso, window_size=80)))
    if alive_nodes_ffo is not None:
        protocol_data.append(('FFO-WSN', smooth_data(alive_nodes_ffo, window_size=80)))
    ...
    if alive_nodes_fuzzy_pso is not None:
        max_rounds = max(max_rounds, len(alive_nodes_fuzzy_pso))
    if alive_nodes_ffo is not None:
        max_rounds = max(max_rounds, len(alive_nodes_ffo))
...
def plot_residual_energy_comparison(residual_energy_improve_leach, residual_energy_leach, residual_energy_heed, residual_energy_d2crp, residual_energy_fuzzy_pso=None, residual_energy_ffo=None):
    import matplotlib.pyplot as plt
    import numpy as np
    plt.figure(figsize=(14, 8))
    plt.rcParams['axes.facecolor'] = 'white'
    ...
    # 添加新的算法数据（如果提供的话）
    if residual_energy_fuzzy_pso is not None:
        protocol_data.append(('Fuzzy+PSO', residual_energy_fuzzy_pso))
    if residual_energy_ffo is not None:
        protocol_data.append(('FFO-WSN', residual_energy_ffo))
    for protocol, data in protocol_data:
        style = plot_styles[protocol]
        if len(data) > 0 and len(rounds) == len(data):
            plt.plot(rounds, data, marker=style['marker'], linestyle='-', color=style['color'], linewidth=2, label=style['label'], markersize=8, markevery=200)
    ...
    if residual_energy_fuzzy_pso is not None:
        max_rounds = max(max_rounds, len(residual_energy_fuzzy_pso))
    if residual_energy_ffo is not None:
        max_rounds = max(max_rounds, len(residual_energy_ffo))
    plt.xticks(range(0, max_rounds + 1, 300))
    plt.xlim(0, max_rounds)
    plt.legend(loc='upper right', fontsize=10,prop={'family': 'Times New Roman'})
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    RUN_PDR_VS_NODES_EXPERIMENT = True # 禁用旧的 PDR vs 节点数实验
    RUN_PDR_OVER_ROUNDS_BY_NODES = False  # PDR 随轮次变化实验控制开关 
    RUN_ENERGY_COMPARISON_EXPERIMENT = False  #剩余能量对比实验控制开关
    # 新增：单节点累计能量消耗曲线实验开关（调用独立脚本）
    RUN_NODE_ENERGY_EXPERIMENT = False
    RUN_NETWORK_LIFETIME_WITH_PSO_EXPERIMENT = False  # 新增：原模糊 vs 模糊+PSO 寿命实验开关
    ...
    # 新增：模糊 vs 模糊+PSO 剩余能量对比实验开关
    RUN_FUZZY_VS_PSO_EXPERIMENT = False
    # 新增：优化前 vs 优化后参数性能对比（的开关
    RUN_SUBEXP1 = False
    ...
    if RUN_NETWORK_LIFETIME_WITH_PSO_EXPERIMENT:
        from network_lifetime_experiment import run_network_lifetime_experiment

        print("\n=== 运行网络寿命实验（原模糊 vs 模糊+PSO） ===")
        run_network_lifetime_experiment()
        print("网络寿命实验完成！")
    ...
    if RUN_NODE_ENERGY_EXPERIMENT:
        # 延迟导入，避免常规运行时的额外依赖
        from node_energy_consumption_experiment import run_cumulative_energy_experiment
        print("\n=== 运行单节点累计能量消耗曲线实验 ===")
        run_cumulative_energy_experiment()
        print("单节点累计能量消耗曲线实验完成！")

    if RUN_FUZZY_VS_PSO_EXPERIMENT:
        print("\n=== 运行模糊逻辑 vs 模糊逻辑+PSO 剩余能量对比实验 ===")
        try:
            # 延迟导入，避免常规运行时的额外依赖
            from node_energy_consumption_experiment import run_fuzzy_vs_pso_energy_experiment
            run_fuzzy_vs_pso_energy_experiment()
            print("模糊逻辑 vs 模糊逻辑+PSO 实验完成！")
        except Exception as e:
            print(f"模糊 vs PSO 实验运行出错: {e}")

    if RUN_SUBEXP1:
        print("\n=== 运行子实验1：FD-MFAR (baseline) vs fuzzy+PSO (optimized) ===")
        try:
            # 延迟导入脚本，避免常规导入时额外依赖
            from experiments_subexp1 import main as run_subexp1_main
            run_subexp1_main()
            print("子实验1完成！")
        except Exception as e:
            print(f"子实验1运行出错: {e}")
